...
import { QUESTIONS, CATEGORY_META, computeResults } from "@/lib/diagnosis-data";
  ...
  const submitQuiz = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("diagnoses")
        .insert({ name: userName, email, answers })
        .select("id")
        .single();
      if (data && data.id) {
          ...
          <div>
            <div style={{ textAlign: "center", paddingTop: 40, marginBottom: 40 }}>
              ...
              <p style={{ color: COLORS.textMuted, fontSize: 15, lineHeight: 1.8, marginBottom: 32 }}>
                14の質問に答えるだけで、<br />あなたの思考・行動パターンの傾向がわかります。<br />
                <span style={{ fontSize: 13 }}>所要時間：約7分</span>
              </p>
              ...
              <div style={{ marginBottom: 12 }}>
                <input type="text" placeholder="お名前" value={userName} onChange={(e) => { setUserName(e.target.value); setFormError(""); }}
                  style={{ width: "100%", padding: "14px 18px", background: COLORS.surfaceLight, border: `1.5px solid ${formError && !userName.trim() ? "#f87171" : COLORS.border}`, borderRadius: 10, color: COLORS.text, fontSize: 15, outline: "none" }} />
              </div>
              <div style={{ marginBottom: 20 }}>
                <input type="email" placeholder="メールアドレス" value={email} onChange={(e) => { setEmail(e.target.value); setFormError(""); }}
                  style={{ width: "100%", padding: "14px 18px", background: COLORS.surfaceLight, border: `1.5px solid ${formError && email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) ? "#f87171" : COLORS.border}`, borderRadius: 10, color: COLORS.text, fontSize: 15, outline: "none" }} />
                {formError && <p style={{ color: "#f87171", fontSize: 12, marginTop: 6 }}>{formError}</p>}
              </div>

              <button onClick={startQuiz} style={{ width: "100%", padding: "16px", background: `linear-gradient(135deg, ${COLORS.accent}, ${COLORS.purple})`, color: "#fff", border: "none", borderRadius: 12, fontSize: 16, fontWeight: 600, cursor: "pointer", letterSpacing: 1 }}>
                診断をはじめる
              </button>
            </div>
          </div>
